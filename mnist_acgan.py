#!/home/liyang/.pyenv/shims/python3.6
# -*- coding: utf-8 -*-
"""
Example from https://github.com/fchollet/keras/blob/master/examples/mnist_acgan.py
...
You should start to see reasonable images after ~5 epochs,
and good iages by ~15 epochs.
You should use a GPU,
as the convolution-heavy operations are very slow on the CPU.
...
def build_generator(latent_size):
    # We will map a pair of (z, L), where z is a latent vector
    # and L is a label drawn from P_c,
    # to image space (..., 1, 28, 28)
    cnn = Sequential()
...
def build_descriminator():
    ...
    # with LeakyReLUs as suggested in the reference paper
    cnn = Sequential()
    ...
    # first output (name=generation) is whether or not
    # the discriminator thinks the image that is being shown is fake,
    # and the second output (name=auxiliary) is the class that
    # the discriminator thinks the image belongs to.
...
if __name__ == '__main__':
    ...
    # get our mnist data
    # and force it to be of shape (..., 1, 28, 28) with range [-1, 1]
    ...
    for epoch in range(epochs):
        ...
        for index in range(num_batches):
            ...
            # using the generated labels as a conditioner.
            # We reshape the sampled labels to be (batch_size, 1)
            # so that we can feed them into the embedding layer as a length one sequence
            ...
            # make new noise.
            # we generate 2 * batch size here 
            # such that we have the generator optimize over an identical number of images as the discriminator
            noise = np.random.uniform(-1, 1, (2 * batch_size, latent_size))
            sampled_labels = np.random.randint(0, num_classes, 2 * batch_size)
            
            # we want to train the generator to trick the discriminator
            # For the generator, we want all the {fake, not-fake} lables to say not-fake
        ...
        # generator an epoch report on performance
